package main

import (
	"fmt"
	"math/rand"
	"strings"
)

var InvSbox = []byte{}

var Sbox = []byte{}

func KEY(key string, rounds int) []string {
	keys := make([]string, rounds)
	keys[0] = key
	for i := 1; i < rounds; i++ {
		keys[i] = ""
		for j := 0; j < len(key); j++ {
			keys[i] += string(Sbox[keys[i-1][j]])
		}
	}
	return keys
}

func AddRoundKey(text, key string) string {
	result := ""
	for i := 0; i < len(text); i++ {
		result += string(text[i] ^ key[i])
	}
	return result
}

func SubBytes(text string) string {
	result := ""
	for i := 0; i < len(text); i++ {
		result += string(Sbox[text[i]])
	}
	return result
}

func ShiftRows(text string, crypt bool) string {
	retention := make([][]byte, 4)
	for i := 0; i < 4; i++ {
		retention[i] = make([]byte, 4)
		for j := 0; j < 4; j++ {
			retention[i][j] = text[i*4+j]
		}
	}
	for i := 0; i < 4; i++ {
		storage := make([]byte, 4)
		for k := 0; k < 4; k++ {
			storage[k] = retention[i][k]
		}
		for j := 0; j < 4; j++ {
			if crypt == true {
				retention[i][j] = storage[(j+i)%4]
			} else {
				retention[i][j] = storage[(j-i+4)%4]
			}
		}
	}
	result := ""
	for i := 0; i < 4; i++ {
		for j := 0; j < 4; j++ {
			result += string(retention[i][j])
		}
	}
	return result
}

func MixColumns(text string, initialization []int) string {
	retention := make([][]byte, 4)
	for i := 0; i < 4; i++ {
		retention[i] = make([]byte, 4)
		for j := 0; j < 4; j++ {
			retention[i][j] = text[i*4+j]
		}
	}
	initializationMat := make([][]byte, 4)
	for i := 0; i < 4; i++ {
		initializationMat[i] = make([]byte, 4)
		for j := 0; j < 4; j++ {
			initializationMat[i][(j+i)%4] = byte(initialization[j])
		}
	}
	for i := 0; i < 4; i++ {
		for j := 0; j < 4; j++ {
			retention[j][i] = (retention[j][0] & initializationMat[j][0]) ^ (retention[j][1] & initializationMat[j][1]) ^ (retention[j][2] & initializationMat[j][2]) ^ (retention[j][3] & initializationMat[j][3])
		}
	}
	result := ""
	for i := 0; i < 4; i++ {
		for j := 0; j < 4; j++ {
			result += string(retention[i][j])
		}
	}
	return result
}

func DeblockAES(text, key string, crypt bool, initialization []int) string {
	rounds := 10
	keys := KEY(key, rounds)
	result := ""
	retention := AddRoundKey(text, key)
	result += retention
	for i := 1; i < rounds-1; i++ {
		retention = SubBytes(retention)
		retention = ShiftRows(retention, crypt)
		retention = MixColumns(retention, initialization)
		retention = AddRoundKey(text, keys[i])
		result += retention
	}
	retention = SubBytes(retention)
	retention = ShiftRows(retention, crypt)
	retention = AddRoundKey(text, keys[rounds-1])
	result += retention
	return result
}

func EncryptDecipherAES(text, key string, crypt bool, initialization []int) string {
	textTime := ""
	for i := 0; i < len(text); i += 16 {
		retentionText := ""
		for j := i; j < i+16; j++ {
			retentionText += string(text[j])
		}
		if crypt == false {
			retentionText = DeblockAES(retentionText, key, crypt, initialization)
			key = retentionText
		}
		if crypt == true {
			retentionKey := retentionText
			retentionText = DeblockAES(retentionText, key, crypt, initialization)
			key = retentionKey
		}
		textTime += retentionText
	}
	return textTime
}

func main() {
	text := "За свою карьеру я пропустил более 9000 бросков,проиграл почти 300 игр. 26 раз мне доверяли сделать финальный победный бросок, и я промахивался. Я терпел поражения снова, и снова, и снова. И именно поэтому я добился успеха."
	fmt.Println("Исходный текст:", text)
	kol := 0
	for len(text)%16 != 0 {
		text += " "
		kol++
	}
	key := ""
	for i := 0; i < 16; i++ {
		key += string(rand.Intn(256))
	}
	fmt.Println("Изначальный ключ:", key)
	out := strings.Builder{}
	out.WriteString(key)
	// запись ключа в файл
	//out.open("KeyOut.txt");
	//out << key;
	//out.close();
	initialization := []int{3, 2, 4, 5}
	text = EncryptDecipherAES(text, key, true, initialization)
	fmt.Println("Зашифрованный текст:", text)
	text = EncryptDecipherAES(text, key, false, initialization)
	for kol != 0 {
		text = text[:len(text)-1]
		kol--
	}
	fmt.Println("Расшифрованный текст:", text)
}


